...
def _on_tool_complete(d: dict) -> None:
    ...
    if result_brief:
        print(f"\n{CYAN}▶{RESET} {start_brief} {DIM}→{RESET} {status_color}{status_icon} {result_brief}{RESET} {DIM}({duration_str}){RESET}", flush=True)
    else:
        print(f"\n{CYAN}▶{RESET} {start_brief} {DIM}→{RESET} {status_color}{status_icon}{RESET} {DIM}({duration_str}){RESET}", flush=True)
