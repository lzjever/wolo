...
class SimpleUI:
    ...
    def __init__(self, manager: ControlManager, terminal: Optional[TerminalManager] = None):
        """
        初始化 UI
    ...
    def print_shortcuts(self):
        """打印快捷键提示"""
        print(f"{Colors.DIM}[快捷键: ^A:插话 ^B:打断 ^P:暂停 ^S:Shell ^L:MCP ^H:帮助 ^C:退出]{Colors.RESET}", flush=True)
    ...
    def print_status(self):
        ...
        print(f"{color}[{icon} {text}]{Colors.RESET} {Colors.DIM}{step_info}{Colors.RESET}", flush=True)
    ...
    async def prompt_for_input(self, message: str = "") -> Optional[str]:
        """
        提示用户输入
    ...
    async def wait_for_input_with_keyboard(self) -> Optional[str]:
        """
        等待用户输入，支持 Esc 取消
        在 WAIT_INPUT 状态下使用
    ...
    def _show_shell_status(self):
        ...
        if running:
            ...
            for shell in running:
                ...
                print(f"  {Colors.WHITE}$ {cmd}{Colors.RESET} {Colors.DIM}({elapsed:.1f}s){Colors.RESET}")
        else:
        ...
        if history:
            ...
            for shell in history[:3]:
                ...
                print(f"  {status_color}{status_icon}{Colors.RESET} {Colors.DIM}$ {cmd}{Colors.RESET} {Colors.DIM}({duration:.1f}s){Colors.RESET}")
                
                # Show last few lines of output
                output_lines = shell.get('output_lines', [])
                if output_lines:
                    # Show last 3 non-empty lines
                    recent = [l for l in output_lines[-5:] if l.strip()][-3:]
                    for line in recent:
...
def create_ui(manager: ControlManager, terminal: Optional[TerminalManager] = None) -> tuple[SimpleUI, KeyboardListener]:
    """
    创建 UI 和键盘监听器
...
_current_ui: Optional[SimpleUI] = None
...
def get_current_ui() -> Optional[SimpleUI]:
    """
    获取当前活动的UI实例。
