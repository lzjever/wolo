    ...
    for field in fields(config_class):
        ...
        if origin is not None:
            ...
            if len(args) == 2 and type(None) in args:
                ...
                field_info["type"] = _python_type_to_json_type(
                    args[0] if args[0] is not type(None) else args[1]
                )
            elif origin is list:
...
def validate_config_value(value: Any, field_type: type, field_name: str) -> tuple[bool, str | None]:
    ...
    if origin is None:
        ...
        if not isinstance(value, field_type):
            return (
                False,
                f"{field_name} must be of type {field_type.__name__}, got {type(value).__name__}",
            )
    elif origin is list:
