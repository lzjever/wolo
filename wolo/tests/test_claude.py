...
from wolo.claude.config import ClaudeConfig, load_claude_config, apply_claude_env
...
from wolo.claude.mcp_config import (
    MCPServerConfig,
    get_claude_desktop_config_paths,
...
""")
    ...
    def test_find_matching_skills(self, tmp_path):
        ...
        # Create skills
        for name, desc in [("ui-design", "UI design tools"), ("api-client", "API client generation")]:
            skill_dir = skills_dir / name
            skill_dir.mkdir()
            (skill_dir / "SKILL.md").write_text(f"""---
name: {name}
...
class TestMCPServerConfig:
    ...
    def test_merge_mcp_configs(self):
        ...
        claude_servers = {
            "web-search": MCPServerConfig(name="web-search", command="npx", args=["claude-version"]),
            "filesystem": MCPServerConfig(name="filesystem", command="npx", args=["fs"]),
        }
        
        wolo_servers = {
            "web-search": MCPServerConfig(name="web-search", command="python", args=["wolo-version"]),
