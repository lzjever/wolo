...
class SessionCommandGroup(BaseCommand):
    ...
    def execute(self, args: ParsedArgs) -> int:
        ...
        if subcommand == "create":
            ...
            print(
                "Available subcommands: create, list, show, resume, watch, delete, clean",
                file=sys.stderr,
            )
...
class SessionResumeCommand(BaseCommand):
    ...
    def execute(self, args: ParsedArgs) -> int:
        ...
        from wolo.session import check_and_set_session_pid, get_session_status, load_session
...
class SessionDeleteCommand(BaseCommand):
    ...
    def execute(self, args: ParsedArgs) -> int:
        ...
        if status.get("is_running"):
            pid = status.get("pid")
            print(
                f"Error: Session '{session_id}' is currently running (PID: {pid})", file=sys.stderr
            )
            print("       Stop the session before deleting it.", file=sys.stderr)
            return ExitCode.SESSION_ERROR
...
class SessionCleanCommand(BaseCommand):
    ...
    def execute(self, args: ParsedArgs) -> int:
        ...
        from wolo.session import delete_session, get_session_status, list_sessions
