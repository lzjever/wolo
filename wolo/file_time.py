...
class FileTime:
    ...
    @classmethod
    def read(cls, session_id: str, file_path: str) -> Optional[float]:
        """
        记录文件读取时间。
    ...
    @classmethod
    def update(cls, session_id: str, file_path: str) -> Optional[float]:
        """
        更新文件的记录时间（写入后调用）。
    ...
    @classmethod
    def get_read_time(cls, session_id: str, file_path: str) -> Optional[float]:
        """
        获取文件的记录读取时间。
        """
        path = Path(file_path)
