...
def _suppress_mcp_shutdown_errors(loop, context):
    ...
    if exc:
        ...
        if isinstance(exc, BaseExceptionGroup):
            ...
            for sub_exc in exc.exceptions:
                ...
                if not ("cancel scope" in sub_exc_str or 
                       "dictionary changed" in sub_exc_str or
                       "stdio_client" in sub_exc_str or
                       "streamable_http_client" in sub_exc_str or
                       "streamablehttp_client" in sub_exc_str or
                       "async_generator" in sub_exc_str or
                       "already running" in sub_exc_str):
                    all_expected = False
                    break
            if all_expected:
    ...
    if asyncgen:
        ...
        if ("stdio_client" in asyncgen_str or 
            "streamable_http_client" in asyncgen_str or 
            "streamablehttp_client" in asyncgen_str):
            logger.debug(f"Suppressing expected async generator shutdown error: {asyncgen_str}")
            return
    
    # Check message for shutdown-related errors
    if "shutdown" in msg.lower() or "closing" in msg.lower() or "unhandled exception" in msg.lower():
        if exc:
            exc_str = str(exc)
            if ("cancel scope" in exc_str or 
                "dictionary changed" in exc_str or
                "stdio_client" in exc_str or
                "streamable_http_client" in exc_str or
                "streamablehttp_client" in exc_str or
                "async_generator" in exc_str):
