...
class TestShouldCompact:
    ...
    def test_single_message_returns_false(self, manager):
        ...
        assert (
            decision.should_compact is False
            or decision.overflow_ratio < manager._config.overflow_threshold
        )
...
class TestCompactExecution:
    ...
    async def test_calculates_tokens_saved(self, manager, mock_config):
        ...
        if result.status == CompactionStatus.APPLIED:
            ...
            assert (
                result.total_tokens_saved == (original_tokens - result_tokens)
                or result.total_tokens_saved > 0
            )
    ...
    async def test_stops_when_under_limit(self, manager, mock_config):
        ...
        if result.status == CompactionStatus.APPLIED:
            ...
            TokenEstimator.estimate_messages(list(result.result_messages))
            # Should be close to or under limit (allowing some margin)
            mock_config.max_tokens - manager._config.reserved_tokens
    ...
    async def test_mixed_message_types(self, manager, mock_config):
        ...
        for i in range(10):
            ...
            if i % 2 == 0:
                ...
                messages.append(Message(role="assistant", parts=[TextPart(text=f"Response {i}")]))
