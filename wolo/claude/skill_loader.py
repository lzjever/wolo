...
class ClaudeSkill:
    ...
    def get_system_prompt(self) -> str:
        ...
        if self.scripts_dir and self.scripts_dir.exists():
            ...
            content = re.sub(
                r"\.claude/skills/[^/]+/scripts/", str(self.scripts_dir) + "/", content
            )
            # Replace .wolo/skills/<name>/scripts/ patterns
            content = re.sub(r"\.wolo/skills/[^/]+/scripts/", str(self.scripts_dir) + "/", content)
    ...
    def matches(self, query: str) -> bool:
        ...
        if any(kw in query_lower for kw in ui_ux_keywords):
            if (
                "ui" in self.name.lower()
                or "ux" in self.name.lower()
                or "design" in self.name.lower()
            ):
                return True
...
def load_skill(skill_dir: Path) -> ClaudeSkill | None:
    """
...
def load_claude_skills(skills_dir: Path | None = None) -> list[ClaudeSkill]:
    """
...
def load_all_skills(
    wolo_skills_dir: Path | None = None,
