...
async def run_watch_mode(session_id: str) -> int:
    ...
    from wolo.cli.events import CYAN, DIM, GREEN, RED, RESET, YELLOW
    ...
    if not watch_server_available:
        ...
        print(
            "       The watch server may have failed to start or the session was started with an older version."
        )
    ...
    try:
        ...
        while True:
            try:
                ...
                if event_type == "connected":
                    ...
                    if brief:
                        print(
                            f"\n{CYAN}▶{RESET} {brief} {DIM}→{RESET} {status_color}{status_icon} {brief}{RESET} {DIM}({duration_str}){RESET}",
                            flush=True,
                        )
                    else:
                        print(
                            f"\n{CYAN}▶{RESET} {tool} {DIM}→{RESET} {status_color}{status_icon}{RESET} {DIM}({duration_str}){RESET}",
                            flush=True,
                        )
        ...
        print("       Make sure the session is running.")
        return ExitCode.SESSION_ERROR
