...
def indentation_flexible_replacer(content: str, find: str) -> Generator[str, None, None]:
    ...
    def remove_common_indent(text: str) -> str:
        ...
        return "\n".join(
            l[min_indent:] if l.strip() else l
            for l in lines
        )
...
def _calculate_similarity(block_lines: list[str], search_lines: list[str]) -> float:
    ...
    # 只比较中间行
    middle_block = [l.strip() for l in block_lines[1:-1]]
    middle_search = [l.strip() for l in search_lines[1:-1]]
...
def find_match(content: str, find: str) -> Optional[str]:
    """
    查找匹配的字符串。
