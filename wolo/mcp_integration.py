...
from .config import Config, ClaudeCompatConfig
from .claude import load_claude_config, load_claude_skills, load_claude_mcp_servers, ClaudeSkill
...
from .tool_registry import ToolSpec, ToolCategory, get_registry
...
def find_matching_skill(query: str) -> Optional[ClaudeSkill]:
    """
...
async def shutdown_mcp() -> None:
    ...
    if _mcp_manager:
        try:
            ...
            error_str = str(e)
            if "cancel scope" in error_str or "stdio_client" in error_str or "async_generator" in error_str:
                logger.debug(f"Ignoring expected MCP shutdown error: {e}")
            else:
                logger.warning(f"Error during MCP shutdown: {e}")
        except Exception as e:
            error_str = str(e)
            if "stdio_client" in error_str or "async_generator" in error_str or "cancel scope" in error_str:
                logger.debug(f"Ignoring expected MCP shutdown error: {e}")
            else:
                logger.warning(f"Error during MCP shutdown: {e}")
        finally:
