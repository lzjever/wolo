...
class RunCommand(BaseCommand):
    ...
    def validate_args(self, args: ParsedArgs) -> tuple[bool, str]:
        ...
        if args.session_options.watch_id:
            return (False, "Error: -w/--watch is a standalone command. Use 'wolo -w <id>' or 'wolo session watch <id>'")
        
        # Need message for non-REPL execution
        if args.execution_options.mode != ExecutionMode.REPL:
            message, has_message = get_message_from_sources(args)
            if not has_message:
                return (False, "Error: A prompt is required. Use 'wolo \"your prompt\"' or 'cat file | wolo'")
    ...
    def execute(self, args: ParsedArgs) -> int:
        ...
        if args.session_options.resume_id:
            ...
            if status.get("is_running"):
                pid = status.get("pid")
                print(f"Error: Session '{resume_id}' is already running (PID: {pid})", file=sys.stderr)
